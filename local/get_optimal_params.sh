...
if [ $# -ne 2 ]; then
  echo "Usage: local/get_optimal_params.sh <param-type> (mono|tri|delta|lda|sat)"
  echo " Param Type:"
  echo "    1      # find optimal number of leaves"
  echo "    2      # find optimal number of gaussians"
fi

if [ $2 == "mono" ]; then # MONO SECTION
	echo
	echo "===== GETTING OPTIMAL GAUSSIANS FOR MONO ====="
	echo
	for i in `seq 1000 100 3000`; do
	  steps/train_mono.sh --boost-silence 1.25 --nj 16 --cmd "$train_cmd" --totgauss $i data/train data/lang exp/mono;
...
elif [ $2 == "tri" ]; then # TRI SECTION
	if [ $1 -eq 1 ]; then
		echo
		echo "===== GETTING OPTIMAL LEAVES FOR TRIPHONE ====="
		echo
		for i in `seq 200 100 500`; do
		  steps/train_deltas.sh --boost-silence 1.25 --cmd "$train_cmd" $i 16000 data/train data/lang exp/mono_ali exp/tri1;
		...
		echo
		for i in `seq 10000 2000 50000`; do
		  steps/train_deltas.sh --boost-silence 1.25 --cmd "$train_cmd" 200 $i data/train data/lang exp/mono_ali exp/tri1;
		  utils/mkgraph.sh data/lang exp/tri1 exp/tri1/graph;
		  steps/decode.sh --config conf/decode.config --nj 16 --cmd "$decode_cmd" exp/tri1/graph data/test exp/tri1/decode;
		  python3 local/get_average_wer.py tri1;
		done
